...
def phasemap_4bit(phasemap, inverted=True):
    """convert a phasemap tensor into a numpy 8bit phasemap that can be directly displayed
    ...
    output_phase = ((phasemap + np.pi) % (2 * np.pi)) / (2 * np.pi)
    if inverted:
        phase_out_8bit = ((1 - output_phase) * 16).round().cpu().detach().squeeze().numpy().astype(np.uint8)  # quantized to 8 bits
    else:
        phase_out_8bit = ((output_phase) * 16).round().cpu().detach().squeeze().numpy().astype(np.uint8)  # quantized to 8 bits
    return phase_out_8bit
...
def random_gen():
    ...
    num_iters = random.randint(100, 2000)
    phase_range = random.uniform(1.0, 4.0)
    learning_rate = random.uniform(0.03, 0.06)

    return num_iters, phase_range, learning_rate