...
class FocalstackDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for image_name in image_names:
            ...
            image = intoamp(image) 
            image = resize_keep_aspect(image, (1024, 1820))
...
class FocalstackDDataset(Dataset):
    def __init__(self, root_dir, depth_dir):
        self.root_dir = root_dir
        self.depth_dir = depth_dir
        self.folder_names = sorted(os.listdir(root_dir))
        self.depth_names = sorted(os.listdir(depth_dir))
    ...
    def __getitem__(self, idx):
        ...
        for image_name in image_names:
            ...
            image = cv2.imread(image_path)
            image = intoamp(image) 
            image = resize_keep_aspect(image, (540, 960))
            image = torch.from_numpy(image).float().cuda()
            images.append(image)
        ...
        depth = cv2.imread(depth_path, 0) /255.
        depth = depth[None, :, :]
        depth = resize_keep_aspect(depth, (540, 960))
        depth = depth[0, :, :]
        # depth = 1 - depth
        ...
        depth[depth <= 0.5] = 0
        depth[(depth > 0.5) & (depth <= 1.5)] = 1
        depth[(depth > 1.5) & (depth <= 2.5)] = 2
        depth[(depth > 2.5) & (depth <= 3.5)] = 3
        depth[(depth > 3.5) & (depth <= 4.5)] = 4
        depth[(depth > 4.5) & (depth <= 5.5)] = 5
        depth[(depth > 5.5) & (depth <= 6.5)] = 6

        depth = depth / 6

        depth = torch.from_numpy(depth).float().cuda()

        # depth_values = torch.tensor([0/6, 1/6, 2/6, 3/6, 4/6, 5/6, 6/6], dtype=torch.float32).cuda()
        depth_values = torch.tensor([4/6, 5/6, 6/6], dtype=torch.float32).cuda()

        mask = torch.stack([(depth == value).float() for value in depth_values])
        print(mask.shape)
        return images, mask
...
class RGBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ## load images
        folder_name = self.folder_names[idx]
        image_path = os.path.join(self.root_dir, folder_name)
        print(image_path)
        image = cv2.imread(image_path)
        image = intoamp(image) 
        # image = pad_crop_to_res(image, (880, 1600))
        image = resize_keep_aspect(image, (880, 1600))
        image = torch.from_numpy(image).float().cuda()
