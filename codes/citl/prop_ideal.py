...
class Propagation(nn.Module):
    ...
    def __init__(self, prop_dist, wavelength, feature_size, prop_type='ASM', F_aperture=1.0,
        ...
        self.bl_asm = min(prop_dist) > 0.11
    ...
    def prop(self, u_in, H, linear_conv=True, padtype='zero'):
        ...
        u_in = tfft.fftshift(tfft.fftn(u_in, dim=(-2, -1), norm='ortho'), (-2, -1))
        u_in = u_in * H
        u_in = tfft.ifftn(tfft.ifftshift(u_in, (-2, -1)), dim=(-2, -1), norm='ortho')

        if linear_conv:
            u_in = utils.crop_image(u_in, input_resolution, pytorch=True, stacked_complex=False)
